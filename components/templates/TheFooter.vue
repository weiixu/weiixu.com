    ...
    <div
      ...
      <div class="container mx-auto px-4">
        <div
          ...
          <nav v-for="(l, title, index) in links" :key="title" class="flex-1 w-full sm:w-auto mb-8 sm:mb-0" :class="{
      ...
      <div class="lg:border-t border-light-border dark:border-dark-border transition-colors duration-300 ease-linear">
        <div class="container mx-auto px-4 flex flex-row items-center content-center justify-between py-4">
          ...
          <small class="flex-1 text-center hidden sm:block">
            <span>© {{ $t(`footer.copyright.startYear`) }}-<span id="copyrightYear">{{ $t(`footer.copyright.currentYear`)
                }}</span> {{ $t(`footer.copyright.desc`) }}</span>
            <a class="inline-block" href="https://beian.miit.gov.cn/" target="_blank" rel="noopener">{{
              $t(`footer.copyright.icp`) }}</a>
          </small>
